...
```bash
# SSH to Mercury server for all key management
ssh root@mercury.ncrmro.com
...
# OR create without expiration (relies on ACLs for security)
headscale preauthkeys create \
  --user ncrmro \
  --ephemeral \
  --tags "tag:github-actions"
...
# Revoke a key
headscale preauthkeys expire key_12345
```
...
- Set up [ACL configuration](../modules/nixos/headscale/acl.hujson) for production use
