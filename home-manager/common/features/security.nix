{
  ...
  config = lib.mkIf config.programs.gnome-keyring-integration.enable {
    ...
    xdg.configFile."docker/config.json" = lib.mkIf config.programs.gnome-keyring-integration.docker.enable {
    ...
    systemd.user.services.bitwarden-session-restore = lib.mkIf config.programs.gnome-keyring-integration.passwordManagers.bitwarden {
      Unit = {
    ...
    systemd.user.services.onepassword-session-restore = lib.mkIf config.programs.gnome-keyring-integration.passwordManagers.onePassword {
      Unit = {
    ...
    home.packages = lib.mkIf (config.programs.gnome-keyring-integration.passwordManagers.bitwarden || config.programs.gnome-keyring-integration.passwordManagers.onePassword) [
      # Bitwarden session helper
      (lib.mkIf config.programs.gnome-keyring-integration.passwordManagers.bitwarden (pkgs.writeShellScriptBin "bw-keyring-login" ''
      ...
      (lib.mkIf config.programs.gnome-keyring-integration.passwordManagers.onePassword (pkgs.writeShellScriptBin "op-keyring-login" ''
    ...
    programs.bash.initExtra = lib.mkIf (config.programs.gnome-keyring-integration.passwordManagers.bitwarden || config.programs.gnome-keyring-integration.passwordManagers.onePassword) ''
      # Load password manager sessions from keyring if available
      ${lib.optionalString config.programs.gnome-keyring-integration.passwordManagers.bitwarden ''
        if [ -f "$HOME/.bitwarden-session" ]; then
          source "$HOME/.bitwarden-session"
